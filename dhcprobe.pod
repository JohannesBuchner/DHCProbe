...
B<dhcprobe> - send a DHCP request to DHCP server to check its configuration
...
Verbose, print more information. Apply multiple times to see DHCP configuration details, connecting progress and the package in hex format.
...
$ dhcprobe -c I<your monitoring host IP address> \
...
When running in very verbose mode, B<dhcprobe> tries to dump all
